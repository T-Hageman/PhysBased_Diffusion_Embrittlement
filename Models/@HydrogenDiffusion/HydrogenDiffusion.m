classdef HydrogenDiffusion < BaseModel
	...
	%gradients (taking into account the current phase-field state). 
    ...
    properties
		...
		phi_Step	%Step in which phasefield is resolved
			...
			if (stp == obj.C_Step)
				...
				parfor n_el=1:size(obj.mesh.Elementgroups{obj.myGroupIndex}.Elems, 1)
					...
					for ip=1:length(w)
						...
						K_cc = K_cc + w(ip) * N(ip,:)'*dNTdC*N(ip,:)/dt + N(ip,:)'*dNTdC_dC*N(ip,:)*(N(ip,:)*(CL-CLOld))/dt;
                    	...
                    	dsh = pfx*((1-obj.kmin)*(1-N(ip,:)*PHI)^2+obj.kmin)*Bstar*XY ...
						    - pfx*(2*(1-obj.kmin)*(1-N(ip,:)*PHI)*squeeze(G(ip,:,:))'*PHI)*(BLine*XY);
